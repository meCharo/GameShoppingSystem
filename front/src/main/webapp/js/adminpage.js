...
$(
    function () {
        ...
        $.post("/api/user/admin/getuser", function (result) {
            for (i = 0; i < 10; i++) {
        ...
        $.post("/api/user/admin/getgames", function (result) {
            for (i = 0; i < 10; i++) {
                if (result.data.game[i]) {
                    ...
                    tr.innerHTML = "<td>" + j + "</td><td id='gname_" + gid + "'>"
                        ...
                        "<p>Developers" + "<input type='text'class='inputmargin_1' id='gamecreater" + "'></p><br>" +
                        "<p>Description" + "<textarea  rows='5'  class='inputmargin_2' id='gamedesc" + "'></textarea></p><br>" +
                        "<p>System Requirement" + "<textarea  rows='5'  class='inputmargin_2' id='sys" + "'></textarea></p><br>" +
                        "</div>" +
                        "<div class='modal-footer' id='savebtn'>" +
                        "<button type='button' class='btn btn-default' data-dismiss='modal'>取消</button>" +
        ...
        $.post("/api/game/kind/all", function (result) {
            var i = 0;
            while (result.data[i]) {
                j = i + 1;
                var tr = document.createElement("tr");
...
function getgeame(pagenum) {
    var gameInfo = document.getElementById("gameInfo");
    gameInfo.innerHTML = "";
    $.post("/api/user/admin/getgames", {page: pagenum}, function (result) {
        for (i = 0; i < 10; i++) {
            if (result.data.game[i]) {
                var tr = document.createElement("tr");
...
function restrict(uid, j) {
    $.post("/api/user/admin/restrictuser", {uid: uid}, function (result) {
        if (result.success) {
            alert("该用户已被限制登录！");
...
function relieve(uid, j) {
    $.post("/api/user/admin/relieveuser", {uid: uid}, function (result) {
        if (result.success) {
            alert("该用户已被恢复登录！");
...
//删除用户
function del(uid, j) {
    $.post("/api/user/admin/deluser", {uid: uid}, function (result) {
        if (result.success) {
            alert("该用户已被删除！");
...
function upgame(gid, j) {
    $.post("/api/user/admin/upgame", {game: gid}, function (result) {
        if (result.success) {
            alert("游戏已上架！")
...
function downgame(gid, j) {
    $.post("/api/user/admin/downgame", {game: gid}, function (result) {
        if (result.success) {
            alert("游戏已下架！")
...
function getgamekind(gid) {
    ...
    $.post("/api/game/admin/getgamekind", {game: gid}, function (result) {
        var i = 0;
        var j = 0;
        while (result.data[i]) {
            while (kinds[j]) {
...
function savekinds() {
    ...
    $.ajax({
        type: "post",
        url: "/api/user/admin/updategamekind",
        traditional: true,
...
function managekind(kid) {
    ...
    var i = 0;
    $.post("/api/user/admin/getallgames", function (result) {
...
function savekindsgames() {
    ...
    $.ajax({
        type: "post",
        url: "/api/user/admin/managerkind",
        traditional: true,
...
function getgameinfo(gid) {
    $.post("/api/game/" + gid, function (result) {
...
function savegameupdate() {
    ...
    $.post("/api/user/admin/updategameinfo", {
        id: gid,
        creater: gamecreater,
        name: gamename,
        desc: gamedesc,
...
function addgame() {
    ...
    xhr.open("POST", "/api/user/admin/addgame", true);
...
function addkind() {
    ...
    $.post("/api/user/admin/addkind", {kind: kind}, function (result) {
        if (result.success) {
            alert("添加成功！");
        }
        else {
