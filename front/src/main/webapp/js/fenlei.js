...
function getgame(page) {
    ...
    var n4 = loc.indexOf("?name=");
    n4 = n4 + 6;
    var nameId = decodeURI(loc.substring(n4)).split("&");
    var kindName = nameId[0];
    var kindId = nameId[1].substring(3);
    url = "/api/game/kind/" + kindId + "/games";
    // console.log(loc);
    // console.log(kindName);
    // console.log(kindId);
    // console.log(url);
    // console.log("hello fenlei");
    $.post(url, {page: page}, function (result) {
        if(result.data){
            ...
            while (result.data.game[k]) {
                ...
                var gid = k;
                var u;
                u = "<a href='/api/game/" + result.data.game[k].id;
