$(
    function () {
        var j = 1;
        $.post("/api/game/everyday", function (result) {
            for (i = 0; i < 5; i++) {
                document.getElementById("gameurl_" + i).href = "/api/game/" + result.data[i].id;
        ...
        $.post("/api/game/newestgames", function (result) {
            var i = 0;
            ...
            while (result.data[i]) {
                ...
                div.innerHTML += "<div class='row rowpy'>" +
                    ...
                    "<a href='/api/game/" + result.data[i].id + "'>" +
        ...
        j = 1;
        var myTabContent = document.getElementById("myTabContent");
        $.post("/api/game/preupgames", function (result) {
            var i = 0;
            var div = document.createElement("div");
        ...
        $.post("/api/game/freegames", function (result) {
            var freegames_1 = document.getElementById("freegames_1");
            for (var i = 0; i < 6; i++) {
                var div = document.createElement("div");
                div.className = "item_1_2";
                div.innerHTML = "<a href='/api/game/" + result.data[i].id + "'><img class='item_1_1' src='/img" + result.data[i].img[0] + "'></a>";
