...
function getpaid(pagenum) {
    $.post("/api/order/paid/" + pagenum, function (result) {
            if (result.data.order[0]) {
                ...
                while (result.data.order[k]) {
                    ...
                    while (result.data.order[k].orderitems[j]) {
                        ...
                        li.innerHTML = "<a role='menuitem' tabindex='-1' href='/api/game/" + gid + "'><div class='divwidth'><div class='setleft'>" + result.data.order[k].orderitems[j].game.name + "</div><div class='setright'>" + "¥" + price + "</div></div></a>";
                        ul.appendChild(li);
                        j++;
                    }
                    k++;
...
function getnotpay(pagenum) {
    $.post("/api/order/notpay/" + pagenum, function (result) {
            if (result.data.order[0]) {
                var i = 0;
...
function getcancel(pagenum) {
    $.post("/api/order/cancel/" + pagenum, function (result) {
            if (result.data.order[0]) {
                var i = 0;
                var mytable = document.createElement("table");
                mytable.id = "myTable2";
...
function cancel(oid) {
    $.post("/api/order/" + oid + "/cancel", function () {
        location.reload();
    })
}
function topay(oid) {
    self.location = '/api/order/' + oid + '/payway'
}
